...
import { Overview, mainSidechainData, tokenTypeData } from './Overview';
...
describe('Overview', () => {
  it('renders inside its page container', () => {
    render(<Overview stats={stats} series={series} granularity="day" />);
    expect(screen.getByTestId('page-overview')).toBeTruthy();
  });

  it('stacks main and sidechain tokens, one segment per series per day', () => {
    const { container } = render(<Overview stats={stats} series={series} granularity="day" />);
  ...
  it('derives main/sidechain totals verbatim from the series', () => {
    expect(mainSidechainData(series)).toStrictEqual([
      { bucket: '2026-07-09', main: 145, sidechain: 27275 },
      { bucket: '2026-07-10', main: 18, sidechain: 0 },
    ]);
  });

  it('derives per-type token data verbatim from the series, including zeros', () => {
    expect(tokenTypeData(series)).toStrictEqual([
      { bucket: '2026-07-09', input: 16, output: 183, cacheRead: 21147, cacheCreation: 6074 },
      { bucket: '2026-07-10', input: 7, output: 11, cacheRead: 0, cacheCreation: 0 },
    ]);
  });
  ...
  it('renders an empty selection without throwing', () => {
    ...
    expect(() => render(<Overview stats={empty} series={[]} granularity="day" />)).not.toThrow();
    expect(screen.getByTestId('page-overview')).toBeTruthy();
  });
});
