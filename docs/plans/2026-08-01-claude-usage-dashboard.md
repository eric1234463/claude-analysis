...
**`sessionsStarted` merge rule** — a distinct count, so it must not be summed across per-file aggregates: one session spans many files (verified: `sessionId` `8b0101e5-…` appears in 16 files — 1 main plus 15 sidechains; 516 files hold 217 sessions, so additive rollup would inflate ~2.4×). Only main-session files emit a session-start event; sidechain files never do. The public `sessionsStarted` field stays a plain number, so the frontend is unaffected.
...
- **Regression locks** (all against checked-in fixtures with hand-computed expected numbers):
  ...
  - Daily totals bucket by local time: a fixture message timestamped `2026-07-08T16:30:00Z` (00:30 local on 2026-07-09, since Hong Kong is UTC+8) yields day key `2026-07-09`, not `2026-07-08`. The early-morning-local window is the only one that discriminates at a positive UTC offset — a late-evening-local timestamp gives the same answer under both rules and would be a lock that cannot fail. Tests set `TZ` explicitly so the expected value does not depend on the machine.
  - The parent+subagent fixture pair yields `sessionsStarted: 1`, not 2 — sidechain files never emit a session-start.
...
7. **Token dedupe rule** — per-file scope, justified structurally: one API request is written by one process into one file, so a `requestId` cannot span files (spot-checks agree, but the structural argument is what makes it safe, and the per-file cache makes cross-file dedupe impossible anyway). Key is `requestId`, falling back to the line `uuid` when absent; keep the *last* usage-bearing occurrence in file order (verified: interim lines carry partial output counts, the last carries the complete figure). _Decided by review finding, 2026-08-01._
8. **Day bucketing** — local time, from a config value defaulting to `Asia/Hong_Kong` rather than the ambient machine zone, applied once in the aggregator; the frontend filters on pre-bucketed day keys and never re-derives days from timestamps. Pinning the zone in config keeps the day-bucketing test's expected value independent of the environment it runs in. Chosen over UTC because a personal dashboard should match the user's working day. _Decided 2026-08-01._
...
10. **Missing agent metadata** — if `agent-<agentId>.meta.json` is absent or unreadable while its `.jsonl` is present, `agentType` degrades to `unknown` and the sidechain's tokens still count. Chosen over skipping the file because the tokens matter more than the label. _Decided by review finding, 2026-08-01._
