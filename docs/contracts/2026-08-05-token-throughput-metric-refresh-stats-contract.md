---
type: contract
title: "Token Throughput Metric — POST /api/stats/refresh"
description: "Documents the manual aggregate refresh response, including mergeable response-throughput counters used by the frontend."
status: completed
...
# Token Throughput Metric Refresh API Contract

**Contract file:** `docs/contracts/2026-08-05-token-throughput-metric-refresh-stats-contract.md`

## Overview

- **Feature:** Response token throughput refresh
- **Purpose:** Re-scan changed transcripts and return a fresh dashboard aggregate containing the counters needed to derive response throughput overall and by lane, model, skill, project, and time bucket.
...
|---|---|---|---|
| POST | `/api/stats/refresh` | Run or join an incremental refresh and return the resulting aggregate | None; local single-user app |
...
Runs the cache-aware stats pipeline and replaces the retained aggregate only after success. Concurrent refresh callers join the same in-flight run. After a failure, the next request may retry.
...
| Field | Type | Notes |
|---|---|---|
| `generatedAt` | `string` | ISO-8601 UTC timestamp for the completed refresh |
...
| Status | When |
|---|---|
| `500` | The refresh pipeline rejects; NestJS returns its standard error payload and the previously retained aggregate is not replaced |
